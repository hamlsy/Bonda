...
import { X, Sparkles, PlusCircle, Calculator, FileText } from 'lucide-react';
...
export const CustomBondModal: React.FC<CustomBondModalProps> = ({
  ...
  return (
    <DialogShell isOpen={isOpen} onClose={onClose} labelledBy="custom-bond-title" describedBy="custom-bond-description" className="max-w-xl">
      <div className="bg-white rounded-xl w-full p-5 sm:p-6 border border-slate-200">
        <div className="flex items-center justify-between pb-4 border-b border-slate-100 mb-5">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 id="custom-bond-title" className="font-extrabold text-slate-900 text-base">새 채권 데모 진단</h3>
              <p id="custom-bond-description" className="text-xs text-slate-500">
                입력값으로 정량 지표를 계산해 이 브라우저에서만 미리 봅니다.
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-xs">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              <label htmlFor="custom-bond-issuer" className="block font-bold text-slate-700 mb-1">발행사명</label>
              <input
                id="custom-bond-issuer"
                type="text"
                value={issuer}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div>
              <label htmlFor="custom-bond-sector" className="block font-bold text-slate-700 mb-1">업종</label>
              <input
                id="custom-bond-sector"
                type="text"
                value={sector}
            ...
            <div>
              <label htmlFor="custom-bond-rating" className="block font-bold text-slate-700 mb-1">신용등급</label>
              <select
                id="custom-bond-rating"
                value={rating}
            ...
            <div>
              <label htmlFor="custom-bond-outlook" className="block font-bold text-slate-700 mb-1">신용전망</label>
              <select
                id="custom-bond-outlook"
                value={outlook}
          ...
          <div className="p-3.5 rounded-2xl bg-emerald-50/60 border border-emerald-200/60 space-y-2.5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
              <div>
                <label htmlFor="custom-coupon-rate" className="block text-xs text-slate-600 font-medium mb-1">
                  표면금리 (%)
                </label>
                <input
                  id="custom-coupon-rate"
                  type="number"
                  step="0.01"
                  value={couponRate}
              ...
              <div>
                <label htmlFor="custom-operating-profit" className="block text-xs text-slate-600 font-medium mb-1">
                  영업이익 (억원)
                </label>
                <input
                  id="custom-operating-profit"
                  type="number"
              ...
              <div>
                <label htmlFor="custom-interest-expense" className="block text-xs text-slate-600 font-medium mb-1">
                  이자비용 (억원)
                </label>
                <input
                  id="custom-interest-expense"
                  type="number"
            ...
            <div className="text-xs text-emerald-800 font-mono">
              자동 계산 ICR: {(operatingProfit / (interestExpense || 1)).toFixed(2)}배
            </div>
          </div>

          {/* Disclosure text */}
          <div>
            <label htmlFor="custom-disclosure" className="font-bold text-slate-700 mb-1 flex items-center gap-1.5">
              <FileText className="w-3.5 h-3.5 text-indigo-600" />
              <span>공시 또는 감사보고서 핵심 사실 (Raw Fact)</span>
            </label>
            <textarea
              id="custom-disclosure"
              rows={3}
